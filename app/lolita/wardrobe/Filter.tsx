...
export default function FilterBar({ currentCategoryClothes, displayedClothes, setDisplayedClothes, isFiltered, setIsFiltered }: {
  ...
  return <div className={`${styles.filter} space-x-4`}>
    <SubstyleFilter text="sweet ♡" substyle={getTagId('Sweet')}/>
    <SubstyleFilter text="gothic ♱" substyle={getTagId('Gothic')}/>
  </div>

  function FilterButton(text: string, onClick: () => void){
    return <button onClick={onClick}>{text}</button>
  }

  function SubstyleFilter({ text, substyle } : {text: string, substyle: number | undefined}){
    const [isSelected, setIsSelected] = useState<boolean>(false)
    console.log(isSelected)

    return <button onClick={() => applySubstyleFilter(substyle, setIsSelected)}>{text}</button>
  }

  function applySubstyleFilter(substyle: number | undefined, setIsSelected: Dispatch<SetStateAction<boolean>>){
